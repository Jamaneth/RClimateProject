...
ggplot(data = globalTemperaturesYear,
  ...
  theme_gdocs() +
  geom_ribbon(data = globalTemperaturesYear %>% filter(TenYearMovingAverage > averageTemp),
    aes(x = year, ymax = TenYearMovingAverage - averageTemp, ymin = 0), fill = "red") +
  geom_ribbon(data = globalTemperaturesYear %>% filter(TenYearMovingAverage < averageTemp),
    aes(x = year, ymax = TenYearMovingAverage - averageTemp, ymin = 0), fill = "blue") +
  theme_economist()
...
for (i in c(10:length(tempsByCountry$TenYearAvg))) {
  if(tempsByCountry$Country[i-9] == tempsByCountry$Country[i]) {
...
#### Quatrième étape : on trace un graphe !
```{r}
ggplot(data = tempsByCountry %>% filter(Country == "Saudi Arabia"),
...
* La misère est-elle moins pénible au soleil ? Lien entre indice global de bonheur et chaleur